/* ============================================================
   ...
   Pure: no DOM, no Node. Lives in shared/ so the record editor and
   the server write path import the same file. Enforcement of the
   result lives with each caller - see checkRules() below.
   ============================================================ */

const FLAT_TYPES = new Set(["text", "memo", "number", "date", "select", "link", "user", "boolean"]);
export const RULE_THENS = new Set(["block_submit", "require_field", "require_approval", "notify"]);
...
/* Throws if `when` will not parse. Used by the publish-time
   meta-schema; evalWhen() still swallows the same errors at runtime
   so a stored garbage rule cannot break a save. */
export function parseWhen(when) {
    if (typeof when !== "string" || !when.trim()) throw new Error("empty when");
    return condAst(when);
}
