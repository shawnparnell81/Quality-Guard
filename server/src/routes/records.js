...
import { formatValue, isEmpty } from "../../../shared/format.js";
import { evaluate as evalExpr } from "../../../shared/expr.js";
import { checkRules } from "../../../shared/rules.js";
import { validateRecord, coerceCell, FLAT_FIELD_TYPES } from "../../../shared/validate.js";
...
records.post("/", requirePermission(createPermissionFor), async (request, response, next) => {
    try {
        ...
        if (formRow.rowCount > 0) {
            ...
            /* Schema + required + rules (audit C1 / P3). Create must
               be clean: every value is new, so every problem counts. */
            const checked = validateRecord(formRow.rows[0].schema, data, { phase: "create" });
            if (checked.problems.length > 0) {
                ...
                return response.status(422).json({
                    ...
                    schema_violations: checked.problems
                });
            }
            ...
            if (checked.missing.length > 0) {
                return response.status(422).json({
                    ...
                    fields: checked.missing
                });
            }

            if (checked.rule_violations.length > 0) {
                return response.status(422).json({
                    error: "This record breaks a form rule",
                    rule_violations: checked.rule_violations,
                    warnings: checked.warnings
                });
            }
            ruleResult = {
                blocked: checked.rule_violations,
                warnings: checked.warnings,
...
records.patch("/:number", requirePermission(editPermissionFor), async (request, response, next) => {
    try {
        ...
        const updated = await withTransaction(async (client) => {
            ...
            /* Schema problems are judged on the incoming patch (narrowed
               to values this write introduces) so a stale cell the
               caller did not touch cannot block the save. Rules run on
               the merged record. Required fields are not re-scanned. */
            const schema = schemaRow.rows[0]?.schema || null;
            const checked = validateRecord(schema, data, {
            ...
            if (checked.problems.length > 0) {
                ...
                return { schemaBlocked: true, schema_violations: checked.problems };
            }

            const ruleResult = checkRules(schema, merged);
...
records.patch("/:number/table/:field", requirePermission(editPermissionFor),
    ...
    try {
        ...
        const outcome = await withTransaction(async (client) => {
            ...
            const merged = { ...record.data, [field.key]: finalRows };

            const checked = validateRecord(schema, merged, {
                phase: "update", prior: record.data
            });
            if (checked.problems.length > 0) {
                log.warn("record_write_schema_mismatch", {
                    phase: "table", number: request.params.number,
                    type: record.type, form_version: record.form_version,
                    problems: checked.problems
                });
                return { schemaBlocked: true, schema_violations: checked.problems };
        ...
        if (outcome.schemaBlocked) {
            return response.status(422).json({
                error: "Some values do not fit the form",
                schema_violations: outcome.schema_violations
            });
        }
        if (outcome.ruleBlocked) {
            return response.status(422).json({
                error: "This record breaks a form rule",
